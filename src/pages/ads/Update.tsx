import { IonBackButton, IonButton, IonCheckbox, IonContent, IonHeader, IonIcon, IonInput, IonItem, IonLabel, IonNote, IonPage, IonSelect, IonSelectOption, IonTextarea, IonTitle, IonToolbar } from "@ionic/react"
import axios from "axios"
import { useEffect, useRef, useState } from "react"
import { BASE_URL } from "../../util/cinfig"
import { arrowBack, checkmarkCircle, chevronBack, closeCircle, closeCircleOutline, images, trashBin } from "ionicons/icons"
...
export const AdUpdate=({product,close})=>{
    ...
    const [ad,setAd]=useState<any>({...product})
    ...
    const addCreate=()=>{
        axios.put('/api/products/'+product.id,{...ad}).then(res=>{
           next()
        }).catch(e=>{
    ...
    return <IonPage>
        <IonHeader>
            <IonToolbar>
            <IonIcon icon={arrowBack} slot="start" onClick={close} />
            <IonTitle>{t("Product update")}</IonTitle>
            </IonToolbar>
            
        </IonHeader>
        <IonContent> <div  className="ion-padding">
 ...
 if(file)
 {
   ...
   data.append('image', file)
     ...
     axios.post('/api/upload',data).then(res=>{
        axios.post('/api/image',{product_id:product.id,image:res.data}).then(response=>{
        const images=ad.images
       images.push(response.data)
       setAd({...ad,Images:[...images],...(!ad.image&&{image:response.data})})
        })
       // const images=ad.images
...
{step==2&& <div style={{display:"grid",gridTemplateColumns:" repeat(3, 1fr) ",gap:10}} >
        ...
        {step==4&&<div>
            {ad.fields?.map(item=><div key={item.id}><IonItem  >
                <IonLabel position="stacked">{item.field?.name_en}</IonLabel>
                {item.lookup_id?<IonSelect value={item.value} onIonChange={(e)=>{
                   const fields= ad.fields.map((field:any)=>{
        ...
        next()
            ...
            {step==5&&<div>
                ...
                imgRef.current?.click()
                    ...
                    Add images <svg xmlns="http://www.w3.org/2000/svg"  width="24" height="24" viewBox="0 0 24 24" strokeWidth="1.5" stroke="#2c3e50" fill="none" strokeLinecap="round" strokeLinejoin="round">
  ...
  <path d="M19 16v6" />
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"auto auto",gap:10}} >
                        {ad.images.map((img,index)=><div key={img} style={{position:"relative"}}>
                            ...
                            <IonItem>
                                <Checkbox color="primary" onClick={()=>setAd({...ad,image:img.image})} checked={ad.image==img.image}></Checkbox>
